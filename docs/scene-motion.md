...
已验证 t15／D3D11 桌面隐藏 Player 的真实离屏路径。geometry shader 后端不构成移动优化；Metal、Vulkan、移动设备、Memoryless 或 GPU 帧时均未验收。便携高效后端、实际时域拒绝／累积、角色分类和完整场景动态画质仍待后续阶段。执行记录见 [渲染记录](rendering.md)。
