...
已验证 t15／D3D11 桌面隐藏 Player 的真实离屏路径；[GTAO 时域模块](scene-gtao-temporal.md)、[场景颜色 TAA](scene-temporal-antialiasing.md) 和 [Motion Blur](motion-blur.md) 已可显式消费对应数据。geometry shader 后端不构成移动优化；Metal、Vulkan、移动设备、Memoryless 或 GPU 帧时均未验收。便携高效后端、完整角色分类和完整场景动态画质仍待后续阶段。执行记录见 [渲染记录](rendering.md)。
