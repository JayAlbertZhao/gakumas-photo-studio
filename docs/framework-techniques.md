...
| P07 | 体积光及动态 DepthShadow；PPT 131 | 默认关闭的 [体积光](volumetric-lighting.md)：有限均匀介质、聚光灯单次散射、真实动态源阴影与 HDR 后处理桥接；不附带第三方插件 | 桌面整图及刚体／蒙皮／cutout 遮挡、质量档已验证；原版参数、逐透明层／低分辨率调度、复杂舞台质量与移动成本仍待完成 |
...
| P09 | 低分辨率透明／扭曲／重特效及上采样；PDF 15、31 | 默认关闭的 [分层透明特效](low-resolution-fx.md)：显式 Full／Half／Quarter 几何顺序、Alpha／Additive／先前背景折射、当前深度 min／max 与完整分辨率边缘重画 | 桌面整图、动态网格／蒙皮／自制粒子与实际相机已验证；P07／P08 联合重特效调度、逐透明层介质、复杂相交层、完整舞台及移动成本仍待完成 |
