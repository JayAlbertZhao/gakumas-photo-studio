...
| E04 | 背景 Deferred／透明 Forward+／Actor Forward；PDF 13、22–29、69 | 可选 SceneDeferredCamera、[全分辨率透明 Forward+](scene-forward-plus.md)；[Low／Heavy 当前光照](fx-forward-lighting.md) 共享灯、GI、Monitor 与阴影；[ShadowMask](scene-baked-shadows.md) 保留独立附件。新增 [完整 SRP Actor Forward](srp-actor-forward.md)：实际场景深度、完整材质／描边／前发 stencil、显式逐 renderer 球谐和场景历史之后的角色合成 | 默认不启用。D3D11／Vulkan 自制材质及一个角色两套服装的前后／侧面／动作已与相同显式输入的普通 Forward 全图对照；自动背景／自阴影、面部贴花／法线更多组合、复杂透明排序、全部角色与移动附件仍待完成。当前独立 HDR／depth 未实现 PDF27 的运动 Half4 或原位 GBuffer4 复用，另见 [场景后端](scene-deferred.md)、[ScreenShadow](scene-screen-shadow.md) 与 [光源阴影](scene-light-shadows.md) |
