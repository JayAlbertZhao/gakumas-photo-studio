...
| E10 | 天空、植被、水、折射、荧光棒等专用表面；PPT 109 | 默认关闭的 [独立水面](scene-water.md)；新增 [自主天空](scene-sky.md)：渐变／Cube／经纬图、太阳盘与可逆相机绑定，原生六面 HDR／mip 直接进入统一反射和水面；保留水面原 Cubemap 字段 | 自制桌面整图、实际天空捕获／反射消费、现有水面蒙皮／深度／光照有对照；植被、钻石折射、荧光棒、完整制作水面、相交介质／时域和移动成本仍待完成。资料只列专用用途，未公开天空／水面公式，不假定原版参数或整体画质一致 |
