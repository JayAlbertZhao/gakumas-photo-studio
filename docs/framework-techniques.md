...
| O02 | Memoryless／RenderPass／SubPass／MRT 复用；PDF 11–12、22、27 | 已有 [TileRenderPass](tile-render-pass.md)；[TileSceneRenderer](tile-scene.md) 消费材质／GI／遮罩与方向光 PBR，五 MRT、GI 附件复用和 256-bit 保守颜色预算；可显式增加当前 R32 深度预处理、四种局部灯／GPU 网格与已有实时阴影，主 pass 不读取绑定中的 D32；新增 SRP 实时 Monitor → 发光网格／三类灯／110 灯网格消费；默认 Built-in 不变 | 位置路径单独增加 8 bytes/pixel 预处理预算，不计作免费片上深度；贴花／反射／Actor／运动的完整 tile 整合、生产内容、Metal／移动驻留和负载测量仍待完成。旧 Deferred 额外 RG8 ABI 不变，新格式有量化差异；Direct 退出警告仍开项，不能据此宣布完整 O02 或移动收益 |
