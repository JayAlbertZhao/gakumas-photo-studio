...
| E04 | 背景 Deferred／透明 Forward+／Actor Forward；PDF 13、22–29、69 | 可选 SceneDeferredCamera 已有；新增 [全分辨率透明 Forward+](scene-forward-plus.md)：GPU bitset→当前透明几何，Point／Spot／Capsule／Area、GI、Monitor及独立主光／局部阴影；角色保留 Forward | 默认不启用；桌面分块／逐灯／独立 CPU 和原生蒙皮对照已有，LowResolutionFx／HeavyFx 光照消费、透明反射／介质整合、ShadowMask 打包、胶囊／面阴影与移动附件仍待完成，另见 [场景后端](scene-deferred.md)、[ScreenShadow](scene-screen-shadow.md) 与 [光源阴影](scene-light-shadows.md) |
...
| P09 | 低分辨率透明／扭曲／重特效及上采样；PDF 15、31 | [独立几何](low-resolution-fx.md) 之外，新增默认关闭的 [联合重特效](heavy-fx.md)：介质／有序几何／镜头元素共用预乘附件，每批一个完整 R8 决策图、一次合成与按需整批重画；折射和尺寸变化形成边界 | 桌面整图、有限纹理、单骨移动与实际双阴影相机链已验证；完整 Forward+、相交层排序、折射介质输运、完整舞台及移动成本仍待完成 |
