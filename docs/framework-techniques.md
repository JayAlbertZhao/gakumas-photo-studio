...
| A06 | 稀疏面部形变、骨与视角修形；PPT 54–65 | 默认 CPU 保留；新增可选 [GraphicsBuffer GPU 后端](gpu-face-deformation.md)，原有权重／眨眼／视角修形共用 | 桌面逐顶点、原生 compute→draw 及一个真实角色六个组合已对照；完整角色／服装、CPU 几何消费者适配、GPU 帧时和移动驱动仍待完成 |
...
| A08 | MotionEffect 材质／Prefab／粒子／定位器；PPT 52、59–62 | 材质替换与图集动画已有；新增显式 [预制体生命周期／Local 粒子／Playable](motion-effects.md)，稳定种子、TRS 挂点、绝对 seek 与立即清理 | 桌面粒子数值／整图重放及图生命周期已验证；当前 CPU／GPU 逐顶点定位、世界轨迹／制作特效、Maya 输入和移动成本仍待完成 |
