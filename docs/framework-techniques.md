...
| P07 | 体积光及动态 DepthShadow；PPT 131 | 默认关闭的 [体积光](volumetric-lighting.md)：有限均匀介质、聚光灯单次散射、真实动态源阴影与 HDR 桥接；新增可选 [低分辨率积分与条件重算](volumetric-reconstruction.md) | 桌面整图、动态遮挡及缩减路径已验证；原版参数、逐透明层输运、完整 P09 联合调度、复杂舞台质量与移动成本仍待完成 |
| P08 | Flare／Ghost；PPT 129、PDF 31 | 默认关闭的 [Flare／Ghost](lens-flares.md)：可配置元素／atlas、当前深度圆盘遮挡、确定性动画、实例化绘制与半／四分宽高效果附件 | 桌面整图和实际相机链已验证；原版处方、完整 P09 调度、半透明多层遮光、复杂舞台与移动帧时仍待完成 |
| P09 | 低分辨率透明／扭曲／重特效及上采样；PDF 15、31 | 默认关闭的 [分层透明特效](low-resolution-fx.md)：显式几何顺序、Alpha／Additive／折射和深度边缘重画；连续体积也有独立缩减／重建路径 | 桌面整图、动态几何及实际相机已验证；几何／P07／P08 联合重特效调度、逐透明层介质、复杂相交层、完整舞台及移动成本仍待完成 |
