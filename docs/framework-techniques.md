...
| O02 | Memoryless／RenderPass／SubPass／MRT 复用；PDF 11–12、22、27 | 已有 [TileRenderPass](tile-render-pass.md)；新增 [TileSceneRenderer](tile-scene.md) 消费既有材质／GI／遮罩与方向光 PBR，五 MRT、内联 R8／GBA332、GI 附件复用与 256-bit 保守颜色预算；默认 Built-in 不变 | 桌面自制材质／蒙皮与原生附件已有对照；局部光／实时阴影／贴花／反射／Actor／运动的完整 tile 整合、Metal／移动驻留和负载测量仍待完成。旧 Deferred 额外 RG8 ABI 不变，新的自主紧凑格式有量化差异；Direct 退出警告仍开项，不能据此宣布完整 O02 或移动收益 |
