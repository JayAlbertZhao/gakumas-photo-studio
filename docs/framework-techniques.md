...
[桌面整帧宿主](desktop-host.md) 显式组合场景、角色、自阴影／背景投影、Planar／SSR、透明特效、DOF 与自主调色，提供不需要私人资产的已编译示例，并提供可选 GBuffer4／硬件深度原位复用。它不替换默认摄影路径；完整运动／TAA 接入、Actor Half4 运动附件与移动实机验收仍未完成，不把模块组合计为整个清单完成。
...
| E04 | 背景 Deferred／透明 Forward+／Actor Forward；PDF 13、22–29、69 | 可选 SceneDeferredCamera、[全分辨率透明 Forward+](scene-forward-plus.md)；[Low／Heavy 当前光照](fx-forward-lighting.md) 共享灯、GI、Monitor 与阴影；[ShadowMask](scene-baked-shadows.md) 保留独立附件。[完整 SRP Actor Forward](srp-actor-forward.md) 提供实际场景深度、完整材质／描边／前发 stencil、逐 renderer 球谐及反射之后合成；可选 [当前角色自阴影／背景 caster](actor-shadows.md)，以及反射历史之后的 GBuffer4／D32S8 原位消费 | 默认不启用。D3D11／Vulkan 自制材质及一个角色两套服装的前后／侧面／动作已与相同显式输入的普通 Forward 全图对照；可选 packed 复用与同格式独立输出对照，默认仍保留独立 Half 输出。完整角色／场景投影联调、面部贴花／法线更多组合、复杂透明排序、全部角色与移动附件仍待完成。PDF27 的运动／depth／MaterialID Half4 仍未实现；名义存储减少不代表移动性能验收。另见 [场景后端](scene-deferred.md)、[ScreenShadow](scene-screen-shadow.md) 与 [光源阴影](scene-light-shadows.md) |
