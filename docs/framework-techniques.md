...
| O02 | Memoryless／RenderPass／SubPass／MRT 复用；PDF 11–12、22、27 | 新增可独立调用的 [TileRenderPass](tile-render-pass.md)：显式 SRP、五 MRT、同像素输入、G2／G4 后续复用、Clear／Load／Store、transient 与 256-bit 保守颜色预算；不自动切换 Built-in 摄影管线 | 当前是调度基础，自制通道方程不代表生产 PBR／Actor；现有 Deferred／角色／反射／运动整合、移动驻留／负载测量与质量回退仍待完成。现有 ShadowMask 额外 RG8 ABI 不变；不得由桌面输出相同推导移动收益 |
