...
任意 shader 全局纹理、未声明的隐藏资源、shader 自身的状态或宿主 context 有效性不能由这些检查完整证明。宿主仍须保证不存在其他读写冲突。深度 input attachment 当前仅允许只读 Vulkan：Metal 存在接口限制，本机 D3D11 仿真捕获中该输入槽为空，不能按有效深度消费验收。其他平台应由宿主选择颜色深度输出或独立 pass，此处没有自动深度复制／平台降级。
