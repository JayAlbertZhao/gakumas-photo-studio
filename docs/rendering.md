...
该入口在读取资产之前进入隔离测试，只生成四边形和测试贴图，使用实际角色 shader 渲染到浮点缓冲。它检查明暗偏移、脸部三角区反射、皮肤遮罩的饱和度响应、发丝/发饰高光分区、RampAdd 的漫反射/镜面反射分工、天光的材质响应、附加光合成、前发的模板/混合/深度行为及阴影的亚纹素过滤。明暗、已知有色基线和非零光照用于拒绝空画面或恒定输出。结果写入 `actor-synthetic.json`；当前代码正常执行完整测试时包含 259 项检查、生成 202 张预览 PNG，通过退出 0，失败退出 2。数值检查读取浮点像素，PNG 只用于查看。需可用 GPU 和普通 Built-in Player，不支持 `-nographics` 或原版 shader 研究构建。
