...
该入口在读取资产之前进入隔离测试，只生成四边形和测试贴图，使用实际角色 shader 渲染到浮点缓冲。它检查明暗偏移、脸部三角区反射、皮肤遮罩的饱和度响应、发丝/发饰高光分区、RampAdd 的漫反射/镜面反射分工及天光的材质响应。明暗、已知有色基线和非零光照用于拒绝空画面或恒定输出。结果写入 `actor-synthetic.json`；当前代码正常执行完整测试时包含 163 项检查、生成 119 张预览 PNG，通过退出 0，失败退出 2。数值检查读取浮点像素，PNG 只用于查看。需可用 GPU 和普通 Built-in Player，不支持 `-nographics` 或原版 shader 研究构建。
...
探针保存 41 张 PNG：近景正面/侧面、描边与前发覆盖开关、明暗边界、世界空间光照（含 `09a-world-light-left`、`09b-world-light-right` 两张强侧光）、点光/聚光、Layer、环境光、全身天光开关、有色边缘光、全身皮肤饱和度及恢复动画后的序列帧，并输出 `rendering-probes.json`（schema `photo-studio.actor-rendering-probes.v2`）。需要自己的兼容数据；不是无资产测试，也不是逐骨骼关键帧编辑器或正式导出器。
