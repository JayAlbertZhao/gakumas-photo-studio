...
该入口在读取资产之前进入隔离测试，只生成四边形和测试贴图，使用实际角色 shader 渲染到浮点缓冲。它检查三个明暗偏移、脸部三角区反射，以及身体/面部材质中零、局部、完整皮肤遮罩的饱和度响应。明暗与已知有色基线用于拒绝空画面或恒定输出。结果写入 `actor-synthetic.json`；当前代码正常执行完整测试时生成 34 张预览 PNG，通过退出 0，失败退出 2。数值检查读取浮点像素，PNG 只用于查看。需可用 GPU 和普通 Built-in Player，不支持 `-nographics` 或原版 shader 研究构建。
