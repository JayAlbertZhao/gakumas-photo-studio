...
原始 VS 的无光栅 stream-output 与本地 RenderDoc 的实际 VS 输出又确认：同一眼高光绘制的 1200 个按三角形顺序展开的顶点具有对应的 UV，但屏幕位置仍相差约 2.658 像素和 −1.650 像素（Y 向上坐标）。历史相机提取把带有偏轴投影分量的行向量当作正交基，用转置代替逆矩阵计算位置；这一假设不成立。已从捕获常量独立解出相机中心和投影偏移，尚待运行时应用与 GPU 复验。当前整数图像配准不能替代正确的相机输入，也不能据此宣称已经消除该误差。
