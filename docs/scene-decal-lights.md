...
`receiverGroup=0` 照亮所有登记场景表面，包括不接受材质贴花的 group 0；1–255 仅匹配相同组。它不照亮未登记的 Forward 角色或透明物体。主 Forward 前景遮挡和背景深度仍保留。阴影默认关闭；需要墙挡住 Spot／Point 时必须显式启用并登记墙为 caster。Point 使用真实六面径向深度及跨面 PCF，见 [阴影契约](scene-light-shadows.md#point-六面阴影)；Capsule／Area 阴影尚未实现。
...
尚未包含原版完整舞台参数、所有角色/透明接收器、Capsule／Area 光源阴影、自动 GI、Forward+ 光列表，以及移动 Memoryless/subpass、带宽与 Vulkan/Metal 实测。不把减少 draw 数量直接写成移动端提速或完整管线已追平。
