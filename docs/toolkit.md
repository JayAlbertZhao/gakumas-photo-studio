...
[TileSceneRenderer](tile-scene.md) 在该调度器上消费现有场景材质、GI 和烘焙遮罩输入，执行五 MRT、方向光 PBR 和 GI 附件到最终颜色的复用。它提供单次材质快照，目标仍归宿主管理；只接入明确支持的场景子集，不自动移植局部灯、反射、角色或运动模块。
...
自制角色材质可直接使用 `MaterialRepairer.FallbackShader()` 返回的 ActorToon shader。Base／Shade／Def、双 Ramp、头发高光、顶点材质行及纹理采样设置见 [材质输入约定](actor-material-inputs.md)；不需要原版 shader 或贴图包，材质与纹理由宿主持有。
...
| 可选自主天空／环境输入 | `SceneSkyCamera` 与 `SceneSkyCapture`；自有渐变／Cube／经纬图、可逆相机绑定、原生 HDR 六面及统一反射／水面消费，见 [天空接入](scene-sky.md) |
