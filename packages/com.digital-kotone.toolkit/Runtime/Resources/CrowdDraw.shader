Shader "Hidden/GakumasPhotoMode/CrowdDraw"
{
    ...
    SubShader
    {
        ...
        Targets Shade(Varying input)
        {
            ...
            o.color=float4(clamp(material.rgb/max(material.a,1e-8)*item.tint.rgb,0,65504),1);o.eye=-mul(_CrowdView,float4(input.world,1)).z;
        ...
        Targets MeshFragment(Varying input){return Shade(input);}
        Targets BillboardFragment(Varying input)
        {
            ...
            input.normal=CrowdRotate(captured.xyz,item.rotationType.xy);input.uv=float4(atlasUv,0,0);return Shade(input);
        }
        ENDCG
        Pass
        {
