#ifndef TOOLKIT_SSR_TRACE_INCLUDED
#define TOOLKIT_SSR_TRACE_INCLUDED
// Independent shared algorithm: raster and compute use identical trace math.
...
float4 TraceSceneReflection(float2 uvInput)
{
    if (_SsrHistory.x < .5) return 0;
    int2 originPixel = min((int2)(uvInput * _SsrSize.xy), (int2)_SsrSize.xy - 1);
    ...
    float3 p = PositionAtDepth(uvInput, originDepth);
    ...
    [loop] for (int iteration = 0; iteration < (int)_SsrTrace.w; iteration++)
    {
        ...
        if (sceneDepth < _SsrFrame.x && max(depth0, depth1) >= sceneDepth - _SsrTrace.y)
        {
                ...
                dot(hitNormal, worldDirection) < -.01)
            {
                ...
                // Projection onto an integer pixel boundary can land one ULP on
            // either side in pixel/compute programs. Snap only this tiny tie
            // band; keep the historical depth rejection unchanged.
            float2 historyPixel = previousUv * _SsrSize.xy;
            float2 boundary = round(historyPixel);
            historyPixel = lerp(historyPixel, boundary, step(abs(historyPixel - boundary), 1e-4));
            if (any(historyPixel < 0) || any(historyPixel >= _SsrSize.xy)) return 0;
            int2 previousPixel = (int2)historyPixel;
