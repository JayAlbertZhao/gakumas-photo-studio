...
`PlanarReflection` 提供默认关闭的镜像捕获与接收面投影，适用于角色／发光网格和镜面宿主。显式指定单 pass 材质和几何输入，输出独立的 HDR 反射与覆盖，支持画外网格、oblique 裁剪、实际深度遮挡及 smoothness mip。它不自动给已有材质增加反射，统一间接光合成仍待完成。详见 [Planar 接入](../../docs/planar-reflection.md)。
