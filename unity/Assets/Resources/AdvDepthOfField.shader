Shader "Hidden/GakumasPhotoMode/AdvDepthOfField"
{
    ...
    SubShader
    {
        ...
        sampler2D _FullCoCTexture;
        sampler2D _DOFBackTexture;
        sampler2D _DOFFrontTexture;
        sampler2D _InflatedCoCTexture;
        ...
        float4 FragCoC(v2f_img input) : SV_Target
        {
            ...
            float coc = (1.0 - FocusDistance / max(eyeDepth, 0.00001)) * MaxCoC;
            float farCoC = saturate(coc);
            float nearCoC = saturate(-coc) * _BokehConstants.z;
            return (farCoC - nearCoC + 1.0) * 0.5;
        }
        ...
        float InflateCoC(float2 uv, float extent)
        {
            float2 sourceTexel = _SourceSize.zw;
            float4 positiveNegative = _InflateTexture.GatherRed(
                sampler_PointClamp, uv + float2(extent, -extent) * sourceTexel);
            float result = max(positiveNegative.x,
                max(positiveNegative.y, positiveNegative.w));

            float4 positivePositive = _InflateTexture.GatherRed(
                sampler_PointClamp, uv + float2(extent, extent) * sourceTexel);
            result = max(result, max(positivePositive.x,
                max(positivePositive.y, positivePositive.z)));

            float4 negativePositive = _InflateTexture.GatherRed(
                sampler_PointClamp, uv + float2(-extent, extent) * sourceTexel);
            result = max(result, max(negativePositive.y,
                max(negativePositive.z, negativePositive.w)));

            float4 negativeNegative = _InflateTexture.GatherRed(
                sampler_PointClamp, uv + float2(-extent, -extent) * sourceTexel);
        ...
        {
            return InflateCoC(input.uv, 4.0);
        }

        float4 FragInflateSecond(v2f_img input) : SV_Target
        {
            return InflateCoC(input.uv, 8.0);
        }
        ...
        {
            float4 centre = tex2D(_MainTex, input.uv);
            ...
            for (int index = 0; index < SAMPLE_COUNT; index++)
            {
                ...
                float4 sampleValue = tex2D(_MainTex,
                    input.uv + kernel.wy * radiusScale);
        ...
        {
            if (tex2D(_InflatedCoCTexture, input.uv).r <= 0.0) return 0.0;
            ...
            float alpha = saturate((accumulation.a / 43.0) * _BokehConstants.w);
        ...
        float4 FragFloodFill(v2f_img input) : SV_Target
        {
            ...
            {
                float4 sampleValue = tex2D(_MainTex,
                    input.uv + _BokehKernel[index].wy * radiusScale);
        ...
        float4 FragPostBlurFar(v2f_img input) : SV_Target
        {
            ...
            float4 s0 = tex2D(_MainTex, input.uv + float2(offset.x, offset.y));
            float4 s1 = tex2D(_MainTex, input.uv + float2(offset.x, -offset.y));
            float4 s2 = tex2D(_MainTex, input.uv + float2(-offset.x, -offset.y));
            float4 s3 = tex2D(_MainTex, input.uv + float2(-offset.x, offset.y));
            float threshold = MaxRadius * 0.1;
            float3 result = centre.rgb;
        ...
        float4 FragPostBlurNear(v2f_img input) : SV_Target
        {
            if (tex2D(_InflatedCoCTexture, input.uv).r <= 0.0) return 0.0;
            float2 offset = _SourceSize.zw;
            float4 result = tex2D(_MainTex, input.uv + float2(offset.x, offset.y));
        ...
        {
            float farCoC = saturate(tex2D(_FullCoCTexture, uv).r * 2.0 - 1.0);
            ...
            {
                float4 front = tex2D(_DOFFrontTexture, uv);
