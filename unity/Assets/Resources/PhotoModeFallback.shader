    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f input, float facing : VFACE) : SV_Target
            {
                ...
                float4 definition = tex2Dbias(_DefTex, actorTextureCoordinate);
                float4 layer = tex2Dbias(_LayerTex, actorTextureCoordinate);
                ...
                if (isHair)
                {
                    float hairHighlightResponse = step(0.15, pow(saturate(dot(receiverNormal, h)), 4.0));
                ...
                {
                    float rampAddX = saturate(definition.r * 2.0 - 1.0 + saturate(receiverNormal.z));
                ...
                if (isEyeHighlight)
                {
                    ...
                    float3 eyeHighlightRimColor = lerp(
                        ...
                        _CapturedRimParameters.x;
                    float3 eyeHighlightLit = baseSample.rgb * 0.96 *
                        _LightColor0.rgb * _CapturedLightColor.rgb;
                ...
                float exactReceiverHalfLambert = dot(capturedReceiverNormal, l) * 0.5 + 0.5;
                float exactSurfaceRamp = saturate(exactReceiverHalfLambert + definition.r - 0.65);
                ...
                float headSurfaceRamp = saturate(
                    dot(headReceiverNormal, l) * 0.5 + definition.r - 0.15);
                ...
                float3 capturedTint = lerp(1.0.xxx, _CapturedShadeTint.rgb,
                                           step(0.01, dot(_CapturedShadeTint.rgb, _CapturedShadeTint.rgb)));
                float3 capturedShade = lerp(baseSample.rgb, shadeSample.rgb * capturedTint, saturate(ramp.a));
                float3 capturedRampBase = baseSample.rgb * ramp.rgb * lerp(1.0.xxx, capturedTint, saturate(ramp.a));
                ...
                float3 ambient = lerp(max(ShadeSH9(float4(n, 1.0)), 0.0),
                                      CapturedActorSH(n) * _CapturedAmbientScale,
                                      capturedSHValid);
                float metallic = isSkin ? 0.0 : saturate(definition.b);
                float smoothness = saturate(definition.g);
                ...
                grazingTerm = lerp(grazingTerm, saturate(smoothness + 0.04),
                                   useType4DiffuseF0);
                float nv = saturate(receiverNormal.z);
                ...
                if (_FaceDebugMode > 20.5 && _FaceDebugMode < 21.5)
                    return float4(capturedBrdf, 1.0);
                float3 lit = diffuse * ambient + capturedBrdf *
                    _LightColor0.rgb * _CapturedLightColor.rgb;
                ...
                if (isEye && _CapturedType4DebugStage > 9.5 &&
                    ...
                    return float4(max(capturedBrdf * _LightColor0.rgb *
